...
resource "aws_appautoscaling_policy" "ecs_cpu_scale_in" {
  name               = "${var.project_name}-ecs-cpu-scale-in"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.ecs.resource_id
  scalable_dimension = aws_appautoscaling_target.ecs.scalable_dimension
  service_namespace  = aws_appautoscaling_target.ecs.service_namespace
  ...
  target_tracking_scaling_policy_configuration {
    ...
    target_value      = var.cpu_scale_in_t_v
...
resource "aws_appautoscaling_policy" "ecs_memory_scale_in" {
  name               = "${var.project_name}-ecs-memory-scale-in"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.ecs.resource_id
  scalable_dimension = aws_appautoscaling_target.ecs.scalable_dimension
  service_namespace  = aws_appautoscaling_target.ecs.service_namespace
  ...
  target_tracking_scaling_policy_configuration {
    ...
    target_value      = var.memory_scale_in_t_v
    scale_in_cooldown = var.scale_in_cooldown
  }
}